...
function ItemDAO(database) {
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
         ...
         *
         */
         var toFind = (category == "All" ? {} : {category: category});
         this.db.collection('item').find(toFind).skip(page*itemsPerPage).limit(itemsPerPage).
          toArray(function(err, result) {
            assert.equal(err, null);
          //  console.log(result);
              callback(result);
    ...
    this.getNumItems = function(category, callback) {
         ...
         */
          var toFind = (category == "All" ? {} : {category: category});
          var cursor =  this.db.collection('item').find(toFind).count();
          cursor.then(function(val) {
             callback(val);
          })
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
         ...
         * db.item.find({ $text: {$search: "formal place"} })
         */
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        *
        */
        var cursor = this.db.collection('item').find({ $text: {$search: query} }).count();
        cursor.then(function(numItems) {
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
         ...
         *
         * db.item.updateOne({"_id": 17}, {$push: {"reviews":{name: "Igor", comment: "test", stars: 4, date: 1456270243800} }})
         */
